...
def ajouter_colonne(fichier_excel, list_colonnes, dico_pays):
    # Modifier les options d'affichage
    pd.set_option('display.max_columns', None)  # Affiche toutes les colonnes
    pd.set_option('display.width', None)  # Ajuste la largeur pour éviter les coupures
    pd.set_option('display.max_colwidth', None)  # Affiche tout le contenu des cellules
    ...
    # Ajouter les colonnes spécifiées avec une valeur initiale de 0
    for colonne in list_colonnes:
        df[colonne] = 0

    # Définir une fonction qui sera appliquée à chaque ligne pour remplir lat et long
    def traiter_ligne(row, dico_pays):
        # Vérifier si le pays existe dans le dictionnaire
        if row.pays in dico_pays:
            df.loc[row.name, 'lat'] = dico_pays[row.pays][0]  # Mise à jour de la latitude
            df.loc[row.name, 'long'] = dico_pays[row.pays][1]  # Mise à jour de la longitude

    # Appliquer la fonction à chaque ligne
    df.apply(traiter_ligne, axis=1, dico_pays=dico_pays)
